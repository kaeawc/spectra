...
func TestParseConnectTypedCalls(t *testing.T) {
	tests := []struct {
		...
		{name: "jvm explicit list", args: []string{"jvm", "list"}, wantMethod: "jvm.list"},
		{name: "jvm inspect", args: []string{"jvm", "4012"}, wantMethod: "jvm.inspect", wantParams: `{"pid":4012}`},
		{name: "jvm explicit inspect", args: []string{"jvm", "inspect", "4012"}, wantMethod: "jvm.inspect", wantParams: `{"pid":4012}`},
		{name: "jvm nested gc", args: []string{"jvm", "gc-stats", "4012"}, wantMethod: "jvm.gc_stats", wantParams: `{"pid":4012}`},
		{name: "jvm nested threads", args: []string{"jvm", "thread-dump", "4012"}, wantMethod: "jvm.thread_dump", wantParams: `{"pid":4012}`},
		{name: "jvm nested heap", args: []string{"jvm", "heap-histogram", "4012"}, wantMethod: "jvm.heap_histogram", wantParams: `{"pid":4012}`},
		{name: "jvm nested vm memory", args: []string{"jvm", "vm-memory", "4012"}, wantMethod: "jvm.vm_memory", wantParams: `{"pid":4012}`},
		{name: "jvm nested telemetry", args: []string{"jvm", "telemetry", "4012", "30"}, wantMethod: "jvm.telemetry.live", wantParams: `{"limit":30,"pid":4012}`},
		{name: "jvm nested jmx status", args: []string{"jvm", "jmx", "status", "4012"}, wantMethod: "jvm.jmx.status", wantParams: `{"pid":4012}`},
		{name: "jvm nested jmx start", args: []string{"jvm", "jmx", "start-local", "4012"}, wantMethod: "jvm.jmx.start_local", wantParams: `{"pid":4012}`},
		{name: "jvm nested jfr summary", args: []string{"jvm", "jfr", "summary", "/tmp/recording.jfr"}, wantMethod: "jvm.jfr.summary", wantParams: `{"path":"/tmp/recording.jfr"}`},
		...
		{name: "jvm telemetry", args: []string{"jvm-telemetry", "4012", "30"}, wantMethod: "jvm.telemetry.live", wantParams: `{"limit":30,"pid":4012}`},
		{name: "telemetry", args: []string{"telemetry", "4012", "30"}, wantMethod: "telemetry.live", wantParams: `{"limit":30,"pid":4012}`},
		...
		{name: "jvm attach", args: []string{"jvm-attach", "4012"}, wantMethod: "jvm.attach", wantParams: `{"pid":4012}`},
		{name: "jvm mbeans", args: []string{"jvm-mbeans", "4012"}, wantMethod: "jvm.mbeans", wantParams: `{"pid":4012}`},
		{name: "jvm mbean read", args: []string{"jvm-mbean-read", "4012", "java.lang:type=Memory", "HeapMemoryUsage"}, wantMethod: "jvm.mbean.read", wantParams: `{"attribute":"HeapMemoryUsage","name":"java.lang:type=Memory","pid":4012}`},
		{name: "jvm mbean invoke", args: []string{"jvm-mbean-invoke", "4012", "java.lang:type=Memory", "gc"}, wantMethod: "jvm.mbean.invoke", wantParams: `{"name":"java.lang:type=Memory","operation":"gc","pid":4012}`},
		{name: "jvm probe", args: []string{"jvm-probe", "4012"}, wantMethod: "jvm.probe", wantParams: `{"pid":4012}`},
		...
		{name: "network capture summarize", args: []string{"network-capture-summarize", "/tmp/c.pcap"}, wantMethod: "network.capture.summarize", wantParams: `{"path":"/tmp/c.pcap"}`},
		{name: "network capture summarize with limit", args: []string{"network-capture-summarize", "/tmp/c.pcap", "limit=100"}, wantMethod: "network.capture.summarize", wantParams: `{"limit":100,"path":"/tmp/c.pcap"}`},
		{name: "network diagnose", args: []string{"network-diagnose", "app_path=/Applications/Slack.app", "pid=4012", "target=api.example.com", "port=443", "timeout_ms=1500"}, wantMethod: "network.diagnose", wantParams: `{"app_path":"/Applications/Slack.app","pid":4012,"ports":[443],"targets":["api.example.com"],"timeout_ms":1500}`},
		...
		{name: "jobs list", args: []string{"jobs"}, wantMethod: "job.list"},
		{name: "job start", args: []string{"job", "start", "storage.system"}, wantMethod: "job.start", wantParams: `{"method":"storage.system"}`},
		{name: "job start with params", args: []string{"job", "start", "storage.byApp", `{"paths":["/Applications/Slack.app"]}`}, wantMethod: "job.start", wantParams: `{"method":"storage.byApp","params":{"paths":["/Applications/Slack.app"]}}`},
		{name: "job get", args: []string{"job", "get", "job-1-abcd"}, wantMethod: "job.get", wantParams: `{"id":"job-1-abcd"}`},
		{name: "job wait", args: []string{"job", "wait", "job-1-abcd"}, wantMethod: "job.get", wantParams: `{"id":"job-1-abcd","wait_ms":30000}`},
		{name: "job wait with seconds", args: []string{"job", "wait", "job-1-abcd", "60"}, wantMethod: "job.get", wantParams: `{"id":"job-1-abcd","wait_ms":60000}`},
		...
		{name: "connect diff", args: []string{"diff", "snap-a", "snap-b"}, wantMethod: "snapshot.diff", wantParams: `{"id_a":"snap-a","id_b":"snap-b"}`},
