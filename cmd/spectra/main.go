// Command spectra diagnoses macOS .app bundles and reports the UI
// framework and runtime that built each one.
//
//	spectra /Applications/Slack.app
//	spectra /Applications/*.app
//	spectra --all                 # scan everything in /Applications
...
func run(args []string) int {
	...
	var paths []string
	if *all {
		paths = append(paths, scanDir("/Applications")...)
		paths = append(paths, scanDir("/Applications/Utilities")...)
	}
	paths = append(paths, fs.Args()...)
	if len(paths) == 0 {
		fmt.Fprintln(os.Stderr, "usage: spectra [--json] [--all] [-v] <App.app> [...]")
		return 2
	}
	sort.Strings(paths)

	results := scan(paths, detect.Options{ScanNetwork: *withNetwork})
...
// scan runs detect.Detect across paths in parallel and returns results in
// the same order as paths. Errors are logged to stderr and the offending
// entry is dropped. Worker count is GOMAXPROCS, capped at len(paths).
func scan(paths []string, opts detect.Options) []detect.Result {
	type indexed struct {
		i   int
		r   detect.Result
		err error
